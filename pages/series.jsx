...
const Movies = ({toggleModal,setToggleModal,setContent}) => {
  ...
  useEffect(()=>{
    const fetchTrending = async () => {
      const res = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=5337a3932c7f5952d92e1ea0248df79e&page=${pageNumber}&with_genres=${genreURL}`);
  ...
  return (
    ...
    <div className={TrendingStyle.title}>Series</div>
      ...
      />
    {
      trending && trending.map(series=>{
        return <SingleMovie key={series.id}
