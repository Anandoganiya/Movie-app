...
const Search = () => {
  ...
  useEffect(()=>{
    const fetchData = async () => {
      const res = await fetch(`https://api.themoviedb.org/3/search/${type ? 'tv':'movie'}?api_key=5337a3932c7f5952d92e1ea0248df79e&page=${pageNumber}&query=${query.search}`);
  ...
  return (
    <main>
        <div className={searchStyle.types}>
            <div onClick={()=>{handleType(0)}} className={`${searchStyle.type} ${type?null:searchStyle.primary}`}>Movies</div>
            <div onClick={()=>{handleType(1)}} className={`${searchStyle.type} ${type?searchStyle.primary:null}`}>TV series</div>
        </div>
        <main className={TrendingStyle.movieLayout}>
    <div className={TrendingStyle.title}>{!type?'Movies':'TV series'}</div>
